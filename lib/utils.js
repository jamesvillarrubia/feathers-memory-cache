...
function getKey (hook, customFn) {
  ...
  if(hook.params.authenticated){
    ...
    a = hook.params[`${entity}`] || {}
    auth = `${qs.stringify(JSON.parse(JSON.stringify(a)), { encode: false })}`;
  }
  ...
  // {prefix}_{group}_{key}_{auth}
  // where each {...} is hashed independently and {group} and {auth} are optional
  // {books}{/1234}{pagination-hook:enabled::books/id?q=xxx&y=zzz}{auth}

  return `${hashCode(hook.path||'')}_${idHash}_${hashCode(path)}_${hashCode(auth)||''}_${hashCode(func)||''}`;

}

// This iterates through ALL of the keys and deletes any with a matching path AND id `{path}{id}`
const purgeId = (scope = 'Global') => (ctx) => {
  const { cache, customFn } = ctx.app.get(`cache_${scope}`)
